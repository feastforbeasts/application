...
export default function ProfilePage() {
  const initials = userProfile.name?.split(" ").map(n => n[0]).join("") || "DU";
    ...
    <AppShell>
      <div className="max-w-4xl mx-auto space-y-8">
        <Card className="shadow-xl overflow-hidden">
          <div className="bg-gradient-to-br from-primary/20 via-background to-accent/20 p-6 md:p-8">
            <div className="flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="text-center md:text-left">
                <h1 className="text-3xl md:text-4xl font-bold text-primary">{userProfile.name}</h1>
                ...
                <Badge variant="outline" className="mt-2 bg-accent/20 border-accent text-accent-foreground font-semibold">{userProfile.role}</Badge>
              </div>
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <StatCard title="Total Donations Made" value={userProfile.totalDonations.toString()} icon={Gift} className="bg-secondary/50" />
              <StatCard title="Reward Points Earned" value={userProfile.totalPoints.toString()} icon={Star} iconColor="text-accent" className="bg-secondary/50" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-lg">
          ...
          <CardContent>
            {userProfile.recentAchievements.length > 0 ? (
